##############################################################
# Description:
#   This optimization model determines the optimal assignment of trucks 
#   from multiple depots to multiple stations to transport various products
#   for each shift. 
#   The objective is to maximize total shipped quantity while minimizing 
#   travel distances, considering supply limits, station capacities and sales, 
#   truck capacities, and driver shift constraints.
#
# Objective:
#   Maximize weighted shipping efficiency:
#     - Reward shipped quantity (Priority)
...
param diff_error >= 0, < 1, default 0.01;           # Any values with difference less than this value will be treated as equal
param diff_squared := diff_error^2;
param max_secondary_objective >= 0;
...
maximize Shipping_Efficiency:
    sum {d in DEPOTS, s in STATIONS, t in TRUCKS, p in PRODUCTS} (capacity_truck[t,p] * assign_truck[d,s,t])
    - (diff_error/(1+max_secondary_objective)) * (sum {d in DEPOTS, s in STATIONS, t in TRUCKS} (distance[d,s] * assign_truck[d,s,t]));
...
s.t. Station_Capacity_Limit {s in STATIONS, p in PRODUCTS}:
    sum {d in DEPOTS, t in TRUCKS} capacity_truck[t,p] 
